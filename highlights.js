...
// activeHlType holds a HL_CATEGORIES key (e.g. 'per', 'date') or null
...
// Shows a small floating "Remove highlight" button near the clicked span
let _removeBtn = null;
...
document.addEventListener('click', e => {
  if (activeHlType) return; // highlighter mode: don't show remove
...
// When a highlighter is active and user selects text, we normally apply a new highlight.
// If user SHIFT+clicks a highlight span, we open a prompt to set a group name on it.
document.addEventListener('click', e => {
  ...
  if (newGroup === null) return; // cancelled
...
// ── Notes assembly ────────────────────────────────────
// Supports data-make-note="false" to suppress note for a group
function _assembleNotes() {
  ...
  // Rule 1: groups with 2+ spans — respects data-make-note="false"
  const groupMap = {};
  document.querySelectorAll('.section-editor, #editor').forEach(container => {
  ...
  Object.entries(groupMap).forEach(([grp, spans]) => {
    ...
    // Check if ANY span in this group has data-make-note="false"
