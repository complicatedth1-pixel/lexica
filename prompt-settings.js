...
Every key fact in the HTML must be highlighted using <span> tags with these exact attributes:
  class="hl-span-[cat]" data-hl-cat="[cat]" data-group="[group-slug]"
 
CATEGORY KEYS — choose the most specific one:
- per     → Person (individual human being)
- org     → Organisation (institution, ministry, company, body)
- place   → Place (country, city, region, river, etc.)
- date    → Date / time period / year
- event   → Named event (war, policy launch, summit, etc.)
- why     → Cause / reason something happened
- effect  → Effect / consequence / outcome
- concept → Abstract concept, theory, principle
- law     → Law, Act, Policy, Scheme, Article
- data    → Number, statistic, percentage, figure, rank
- note    → Standalone insight/fact that doesn't fit a group (no data-group needed)
 
GROUP SLUG (data-group):
- A short kebab-case slug connecting related highlights, e.g. "kalinga-war", "rbi-policy-2024"
- All spans sharing a group slug auto-assemble into a revision note in the right panel
- Groups with 2 or more highlights become notes — groups with only 1 highlight do NOT
- So: only assign data-group when there are genuinely 2+ related highlights to connect
- If a fact stands alone with no connected facts → use class="hl-span-note" data-hl-cat="note" (no data-group)
 
data-make-note ATTRIBUTE:
- Default behaviour: any span with data-group gets included in the notes panel
- To EXCLUDE a span from the notes panel (while keeping its highlight colour), add: data-make-note="false"
- When to use data-make-note="false":
  → Same entity referenced twice in the same sentence (e.g. "RBI was founded by RBI Act 1934" — second "RBI" mention)
  → Repeated mentions of the same person/org in close proximity where including both would make the note redundant
  → A span that contextually belongs to a group colour-wise, but adds no new information to the revision note
  → Decorative/connective highlights that don't carry standalone recall value
 
HIGHLIGHT RULES SUMMARY:
1. Highlight every key fact — names, dates, numbers, causes, effects, laws, concepts, organisations, places
2. Pick the most specific category key from the list above
3. Assign data-group only when 2+ highlights are genuinely connected and form a useful revision note together
4. Single isolated facts → use hl-span-note / data-hl-cat="note" with NO data-group
5. Redundant repeat mentions → add data-make-note="false" to suppress from notes without removing highlight
6. Never leave a key fact unhighlighted just because you're unsure of the group — use "note" as fallback
7. Groups should be meaningful: "maurya-empire", "gst-implementation", "kalinga-war" — not vague like "economy" or "history"
 
EXAMPLES:
<span class="hl-span-per" data-hl-cat="per" data-group="ashoka-kalinga">Ashoka</span> fought the
<span class="hl-span-event" data-hl-cat="event" data-group="ashoka-kalinga">Kalinga War</span> in
<span class="hl-span-date" data-hl-cat="date" data-group="ashoka-kalinga">261 BCE</span>, which caused
<span class="hl-span-effect" data-hl-cat="effect" data-group="ashoka-kalinga">~100,000 deaths</span>.
 
Standalone note (no group):
<span class="hl-span-note" data-hl-cat="note">Ashoka is the only ancient emperor to publicly renounce war after victory.</span>
 
Suppressing a redundant mention:
The <span class="hl-span-org" data-hl-cat="org" data-group="rbi-founding">RBI</span> was established by the
<span class="hl-span-law" data-hl-cat="law" data-group="rbi-founding">RBI Act 1934</span>.
<span class="hl-span-org" data-hl-cat="org" data-group="rbi-founding" data-make-note="false">RBI</span> commenced operations on 1 April 1935.
...
13. HIGHLIGHT RULES: Follow all HIGHLIGHT INSTRUCTIONS above exactly. Every key fact wrapped in <span>. Correct category. Meaningful group slugs. data-make-note="false" on redundant repeat mentions. Standalone facts use hl-span-note with no data-group. Groups only when 2+ connected highlights exist.`;
}
